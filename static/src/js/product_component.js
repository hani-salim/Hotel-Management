...
export class ProductComponent extends Component {
    ...
    async loadProducts() {
        try {
            const products = await this.orm.searchRead(
                "product.product",
                [["sale_ok", "=", true], ["type", "=", "product"]], // تأكد من أن النوع هو 'product' وليس 'service'
    ...
    async openModal(product) {
        ...
        // Get latest product data to check quantity
   ...
   async finalizeOrder() {
    ...
    this.state.isProcessing = true;
     this.state.isProcessingInvoice = true;
        ...
        // إنشاء طلب البيع مع origin فريد
        ...
        // الحصول على حركة المخزون المرتبطة بطلب البيع باستخدام sale_order_id
        ...
        if (pickingData.length > 0) {
            try {
                ...
                // إذا كانت النتيجة تحتاج إلى إجراء إضافي (مثل التحقق من العمليات الجزئية)
            ...
            console.error("No picking created for the order");
            this.notification.add("Inventory movement not created automatically", {
                type: "warning",
                buttons: [{
                    name: "Check Order",
    ...
    // إعداد بيانات الفاتورة
        ...
        this.notification.add("Order completed but invoice creation failed", {
            type: "warning",
            buttons: [{
                name: "Try Again",
